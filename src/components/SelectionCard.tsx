...
interface SelectionCardProps {
  title: string;
  icon?: React.ReactNode; // Made icon optional with the ? operator
  selected?: boolean;
  onClick?: () => void;
}
...
const SelectionCard: React.FC<SelectionCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <h3 className={`text-lg font-medium text-center text-gray-800 ${icon ? 'mt-2 sm:mt-4' : 'my-auto'} w-full px-2 break-words line-clamp-2 overflow-hidden`}>
        {title}
      </h3>
    </div>
  );
