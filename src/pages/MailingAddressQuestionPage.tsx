...
const YesSVG = ({ selected }: { selected?: boolean }) => (
  <div className={`p-2 rounded-full transition-colors ${selected ? 'bg-blue-50' : ''}`}>
    ...
    >
      <div className="flex flex-col items-center justify-center flex-grow w-full max-w-3xl mx-auto py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full max-w-2xl mb-12">
          <SelectionCard
            title="Yes"
            icon={<YesSVG selected={selectedOption === 'yes'} />}
            selected={selectedOption === 'yes'}
            onClick={() => handleOptionClick('yes')}
          />
          <SelectionCard
            title="No"
            icon={<NoSVG selected={selectedOption === 'no'} />}
